# The Hollow Spire — Book 01 Assembly Notes

**Version:** v0.01  
**Purpose:** Working notes for assembling Book 01 into a publication-ready Homebrewery source.

## Current Assembly Goal

Create a stable Book 01 publication structure before importing full content. This keeps the project from becoming a loose collection of completed assets and makes every future pass auditable.

## Immediate Build Order

1. Establish Book 01 Homebrewery source skeleton.
2. Establish publication audit.
3. Establish asset checklist.
4. Import Floor 1 core canon.
5. Import Floor 1 encounter sequence EN-001 through EN-008.
6. Import Nib.
7. Import Floor 1 bestiary.
8. Import Discovery Archive and Recovery Registry entries relevant to Floor 1.
9. Audit all maps for viewability and player safety.
10. Audit all art/reference needs.
11. Resolve Flooded Approach and Emberlight placement.
12. Decide whether Floors 2–10 need full rewrite or recovery pass.

## Homebrewery Formatting Standards

Use the following conventions unless replaced by a later style guide:

- One major section per page break during drafting.
- Use boxed text sparingly for scenes the GM is likely to read aloud.
- Keep secrets in GM-facing paragraphs, not player handout sections.
- Use consistent labels for encounter IDs, creature names, map IDs, and registry entries.
- Every encounter should include:
  - Purpose
  - Setup
  - Read-aloud text if useful
  - Keyed area or trigger
  - Creatures or NPCs
  - Checks and DCs
  - Outcomes
  - Discoveries
  - Recoveries/rewards
  - Warden Ledger implications where relevant

## Publication Rules to Preserve

- The Hollow Spire should feel hostile, ancient, and rule-bound.
- The party should not be able to bypass the campaign through simple teleportation or planar escape.
- Maps should be artifacts of perspective, not omniscient truth.
- Recurrence should be unsettling but meaningful.
- The Seven Seals should feel like campaign-scale milestones.
- The Warden Ledger should track moral consequence without reducing roleplay to a simple score.

## Known Risks

| Risk | Impact | Mitigation |
|---|---|---|
| Floor 1 assets marked complete may lack reference art | Publication delay | Run asset-by-asset art audit before final layout. |
| Maps may not be readable at book scale | Layout delay | Test each map at target page size. |
| Floors 2–10 story material may be missing | Book 01 scope risk | Decide between recovery, rewrite, or phased publication. |
| Canon may have drifted across files | Continuity risk | Use publication audit before importing final prose. |
| Homebrewery formatting may become hard to maintain | Production risk | Keep source modular and use consistent section templates. |

## Next Patch Recommendation

After this structure is merged, the next patch should import Floor 1 core canon and convert EN-001 into the first fully formatted publication encounter. This creates a model for EN-002 through EN-008.

# Master Recovery Registry v0.02

## Iron Rat Recoveries

1. Iron Rat Teeth — Iron Rat — Common
2. Rust Gland — Iron Rat — Common
3. Copper Whiskers — Iron Rat — Common
4. Core Fragment — Iron Rat — Uncommon
5. Colony Marker — Alpha Iron Rat — Uncommon
6. Alpha Node — Alpha Iron Rat — Rare
7. Rustmother Egg Fragment — Rustmother — Rare

## Broken Custodian Recoveries

8. Alloy Scraps — Custodian — Common
9. Actuator Spring — Custodian — Common
10. Depleted Power Cell — Custodian — Common
11. Servo Motor — Custodian — Uncommon
12. Optic Lens — Custodian — Uncommon
13. Gear Cluster — Custodian — Uncommon
14. Core Stabilizer — Custodian — Rare
15. Control Node — Custodian — Rare
16. Memory Fragment — Custodian — Rare
17. Protocol Heart — Custodian — Very Rare
18. Warden Sigil — Custodian — Very Rare
19. Overcharge Capacitor — Custodian — Very Rare

## Environmental Recoveries

20. Scrap Metal — Black Cells — Environmental
21. Broken Tools — Black Cells — Environmental
22. Explorer Token — Black Cells — Environmental
23. Survey Chalk — Rotunda — Environmental
24. Rusted Survey Tags — Rotunda — Environmental
25. Mechanism Shard — Rotunda — Environmental
26. Explorer Sketches — Nib's Journal — Historical entry; not an EN-003 reward (CAN-001)
27. Improvised Route Map — Nib's Notes — Historical entry; not an EN-003 reward (CAN-001)
28. Maintenance Tools — Maintenance Hall — Environmental

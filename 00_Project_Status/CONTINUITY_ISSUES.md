...
|---|---|---|---|
| CONT-001 | Nib introduction and role | Corrected; visual follow-up | CAN-001: Rotunda survivor. Core, NPC, encounter, reward references aligned; inspect map/visual labels separately. |
| CONT-002 | Seven Seals lists | Corrected | DOC-001: v2.0 authority; provisional chain removed from live skeleton. |
| CONT-003 | Region floor order | Corrected | DOC-001: indexed design lock; stale progression retired. |
| CONT-004 | Conflicting next tasks | Corrected | PROJECT_STATE is current; Chapter 00 remains on hold. |
| CONT-005 | Repository presence treated as approval | Corrected | Explicit topic authority and status index. |
...
These are deliberate design unknowns, not errors to auto-correct:
- Whether the breach caused the party's release.
- Exact identity, nature, and final revelation of the hidden creature.
- Keeper / Saint Lantern identity.
- Unrevealed recurrence and Warden truths.

## Closing an issue

Record the deciding source, update affected dependents, verify them, and link a decision ID. An audit report alone is not evidence that the underlying content was corrected.

## Legacy IDs

Do not merge DA evidence-object IDs into Discovery award IDs without source support. Known award equivalences live in [Discovery_Aliases.json](../07_Registries/Discovery_Aliases.json); other mappings remain unreconciled.
